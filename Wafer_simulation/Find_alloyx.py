...
space = 0.05
delta_E = 0  # could be DC bias or whatever
...
class DEVICE_FILE:
    ...
    # def __init__(self, Gamma_path, input_path, eigen_path):
    #     self.input_file = nn.InputFile(input_path)
    #     width1 = self.input_file.variables['surface_cap_width'].value
    #     width2 = self.input_file.variables['upper_AlGaAs_doping_layer_width'].value
    #     width3 = self.input_file.variables['upper_AlGaAs_modulation_spacer_width'].value
    #     self.QW1_start = width1 + width2 + width3 - 0.5  # -0.5 to allow some space
    #     self.Gamma_file = nn.DataFile(Gamma_path, product='nextnano++')
    #     self.eigen_file = nn.DataFile(eigen_path, product='nextnano++')
    #     self.x = self.Gamma_file.coords['x']
    #     self.Gamma = self.Gamma_file.variables['Band_Edge'].value
    #     self.E_1 = self.eigen_file.variables['E_1'].value[0]
...
DEV10_prob_path = str(r'E:\OneDrive--University of Cambridge\OneDrive - University of Cambridge'
                       r'\Documents\nextnano\Output\{type}full\bias_00000\transmission_cbr_Gamma1.dat'.format(type=Model))
...
DevTEST_Gamma_path0 = str(r'E:\OneDrive--University of Cambridge\OneDrive - University of Cambridge'
                          r'\Documents\nextnano\Output\{type}DEVf=20\bias_00000\bandedge_Gamma.dat'.format(type=Model))
DevTEST_prob_path0 = str(r'E:\OneDrive--University of Cambridge\OneDrive - University of Cambridge'
                          r'\Documents\nextnano\Output\{type}DEVf=20\bias_00000\transmission_cbr_Gamma1.dat'.format(type=Model))
...
diff_array = []
var1_name = 'alloy1_x'
limit = np.linspace(0.11, 0.41, 61)
...
for var in limit:
    ...
    ModTEST.input_file.save(r'F:\NextNano Data\TLL'
                            r'\ModTEST{bias}_{var1}.in'.format(bias=bias_type, var1=var1_name + '=' + str(var1)), automkdir=False,
                            overwrite=True)
...
if isvarymodeln20:
    ...
    DEV_result_input_path = str(r'F:\NextNano Data\TLL'
                                r'\{type}DEVf=20_{var1}.in'.format(type=Model, var1=var1_name + '=' + str(alloyx_min)))
...
t_GaAs = lambda t: ((0.33 - x_av) / x_av) * t  # characteristic equation so that x_av of wafer(real) agrees with x_av(
# theoretical)
...
REAL.input_file.save(r'F:\NextNano Data\TLL\{type}FIN_MODREAL_{var2}_{var3}.in'
                     .format(type=Model, var2=var2_name + '=' + str(round(var2, 4)),
                             var3=var3_name + '=' + str(round(var3, 4))),
...
REAL_prob_path = str(r'E:\OneDrive--University of Cambridge\OneDrive - University of Cambridge'
                     r'\Documents\nextnano\Output\{type}FIN_MODREAL_{var2}_{var3}'
                     r'\bias_00000\transmission_cbr_Gamma1.dat'
                     .format(type=Model, var2=var2_name + '=' + str(round(var2, 4)),
                             var3=var3_name + '=' + str(round(var3, 4))))
...
DevTEST0.input_file.save(r'F:\NextNano Data\TLL\{Type}FIN_DEVREAL_{var2}_{var3}.in'
                         .format(Type=Model, var2=var2_name + '=' + str(round(var2, 4)),
                                 var3=var3_name + '=' + str(round(var3, 4)))
                         , automkdir=False, overwrite=True)
DevTEST0.input_file.execute()
DevTEST0_input_path = str(r'F:\NextNano Data\TLL\{Type}FIN_DEVREAL_{var2}_{var3}.in'
                          .format(Type=Model, var2=var2_name + '=' + str(round(var2, 4)),
                                  var3=var3_name + '=' + str(round(var3, 4))))
DevTEST0_Gamma_path = str(r'E:\OneDrive--University of Cambridge\OneDrive - University of Cambridge'
                          r'\Documents\nextnano\Output\{type}FIN_DEVREAL_{var2}_{var3}'
                          r'\bias_00000\bandedge_Gamma.dat'
                          .format(type=Model, var2=var2_name + '=' + str(round(var2, 4)),
                                  var3=var3_name + '=' + str(round(var3, 4))))
...
print("Transmission probability of new wafer is {x:.3e} ".format(x=prob_wafer))
print("Transmission probability of new wafer (REAL) is {x:.3e} ".format(x=prob_wafer_real))
print("PLOTTING GRAPHS...")

# PLOTTING SECTION

fig, ax = plt.subplots(2, 2)
ax[0, 0].plot(limit, diff_array, 'darkgreen')
ax[0, 0].scatter(alloyx_min, diff_min, color='r', label='Minimum diff ({x},{:.3e})'.
                 format(diff_min, x=round(alloyx_min, 4)))
ax[0, 0].legend(loc='upper right')
ax[0, 0].set_xlabel("Alloy ratio")
ax[0, 0].set_ylabel("Abs diff in ground state E trans prob")

ax[0, 1].plot(t_AlGaAs, t_GaAs(t_AlGaAs), label=r'$t_{GaAs}=\frac{0.33-x_{av}}{x_{av}}t_{AlGaAs}$')
ax[0, 1].plot(t_AlGaAs, y(t_AlGaAs), label=r'constraint $t_1+t_2=1.389$')
ax[0, 1].scatter(intersect_x, intersect_y, color='r', label=r'Intersection ({x},{y})'
                 .format(x=round(intersect_x, 4), y=round(intersect_y, 4)))
ax[0, 1].legend(loc='lower right')
ax[0, 1].set_title(r'x={x},x_av={d}'.format(x=alloyx_min, d=round(x_av, 4)))
ax[0, 1].set_xlabel('t_AlGaAs')
ax[0, 1].set_ylabel('t_GaAs')

ax[1, 0].plot(MODREAL_result.E, MODREAL_result.prob, 'red', label='New wafer REAL (x_av = {ratio})'
              .format(ratio=round(x_av, 4)))
ax[1, 0].plot(MOD_result.E, MOD_result.prob, '-.', color='indigo',
              label='New wafer (x = {ratio})'.format(ratio=round(alloyx_min, 4)))
ax[1, 0].plot(MODEL10.E, MODEL10.prob, '--', color='coral', label='Existing wafer')
ax[1, 0].legend(loc='lower right')
ax[1, 0].set_title(r'{T} 2-Well Model Transmission profile [1->2], prob = {p:.3e}, prob(REAL) = {x:.3e}'
                   .format(T=Model, p=prob_wafer, x=prob_wafer_real))
ax[1, 0].set_xlabel('Energy(eV)')
ax[1, 0].set_ylabel('Transmission Probability')

ax[1, 1].plot(DEVREAL_result.E, DEVREAL_result.prob, 'magenta', label='New wafer REAL (x_av = {ratio})'
              .format(ratio=round(x_av, 4)))
ax[1, 1].plot(DEV_result.E, DEV_result.prob, '-.', color='seagreen',
              label='New wafer (x = {ratio})'.format(ratio=round(alloyx_min, 4)))
ax[1, 1].plot(DEV10.E, DEV10.prob, '--', color='cyan', label='Existing wafer')
ax[1, 1].legend(loc='lower right')
ax[1, 1].set_title(r'{Type} Transmission profile [1->2]'.format(Type=Model))
...
print("Resultant Device (REAL) data file saved in: {path}".format(path=DevTEST0_Gamma_path))
print("Resultant Model (REAL) data file saved in: {path}".format(path=REAL_prob_path))
print("Final Device_E1 path:{path}".format(path=Dev_result_path))
print("Final Model_E1 path:{path}".format(path=MOD_result_path))
print("Final real Device_E1 path:{path}".format(path=DevTEST0_Gamma_path))
print("Final real Model_E1 path:{path}".format(path=REAL_prob_path))

if isvarymodeln20:
    fig_path = str(r'E:\OneDrive--University of Cambridge'
                   r'\OneDrive - University of Cambridge\Desktop\Research\MBE Wafer Growth'
                   r'\Python figs\{Type}alloy_x={factor}_model(un-calibrated density).png'.format(Type=Model, factor=round(alloyx_min, 4)))
    fig.savefig(fig_path, dpi=500)

    print("Figure saved in: {path}".format(path=fig_path))
else:
    fig_path = str(r'E:\OneDrive--University of Cambridge'
                   r'\OneDrive - University of Cambridge\Desktop\Research\MBE Wafer Growth'
                   r'\Python figs\{Type}alloy_x={factor}_fixedmodel.png'.format(Type=Model, factor=round(alloyx_min, 4)))
    fig.savefig(fig_path, dpi=500)

    print("Figure saved in: {path}".format(path=fig_path))

# calculate charge density in new wafer (real)
